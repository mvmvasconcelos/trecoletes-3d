...
export default function ChaveiroSimples() {
    ...
    useEffect(() => {
        axios.get(`${API_BASE}/api/models/chaveiro_simples/config`)
            .then(res => {
                ...
                initial['extrusor_outline'] = 2;

                setParams(initial);
                const initOpen: Record<string, boolean> = {};
                cfg.sections?.forEach((s: any) => { initOpen[s.name] = true; });
    ...
    const handleClearCache = async () => {
        ...
        try {
            ...
            setTmfUrl(null); setBaseUrl(null); setLettersUrl(null); setOutlineUrl(null); setFromCache(null);
    ...
    const handleGenerate = async () => {
        ...
        setError(null); setBaseUrl(null); setLettersUrl(null); setOutlineUrl(null); setTmfUrl(null); setFromCache(null);
        try {
            ...
            if (res.data?.files) {
                ...
                if (res.data.files.outline) setOutlineUrl(`${API_BASE}${res.data.files.outline}`);
        ...
        switch (p.type) {
            ...
            case 'color': {
                const extField = p.id === 'base_color' ? 'extrusor_base' :
                    p.id === 'letters_color' ? 'extrusor_letras' : 'extrusor_outline';
                const extVal = params[extField] ?? (p.id === 'base_color' ? 1 : p.id === 'letters_color' ? 4 : 2);
                return (
                    <BambuColorPicker
                    ...
                    {config && (
                        <> 
                            <div className="space-y-4 pb-1">
                                ...
                                {config.parameters?.filter((p: any) => p.id.startsWith('text_')).map(renderParam)}
                            </div>
                        ...
                        {batchProgress && (
                            <div className="space-y-1">
                                <div className="w-full bg-neutral-800 rounded-full h-2 overflow-hidden">
                                    <div className="bg-violet-500 h-2 rounded-full transition-all duration-500" style={{ width: `${batchProgress.total > 0 ? (batchProgress.done / batchProgress.total) * 100 : 0}%` }} />
                                </div>
            ...
            <section className="flex-1 p-4 relative min-w-0 min-h-0 flex flex-col gap-3">
                ...
                {tmfUrl && (
                    <div className="flex-shrink-0 flex flex-col items-center gap-1">
                        ...
                        <button
                            onClick={() => downloadBlob(tmfUrl!, 'chaveiro_simples_all.3mf')}
