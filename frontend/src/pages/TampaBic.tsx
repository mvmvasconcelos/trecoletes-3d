...
function buildPrimaryFilename(primaryText: string | undefined) {
    const normalized = (primaryText ?? '')
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '_')
    ...
    return `${normalized || 'tampa_bic'}.3mf`;
}

export default function TampaBic() {
    ...
    useEffect(() => {
        axios.get(`${API_BASE}/api/models/tampa_bic/config`)
            .then(res => {
                const cfg = res.data;
                setConfig(cfg);
                ...
                cfg.sections?.forEach((s: any) => {
                    initOpen[s.name] = s.collapsed !== undefined ? !s.collapsed : !s.name.includes('Secundária');
                });
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const res = await axios.post(`${API_BASE}/api/generate_parametric/tampa_bic`, form);
            if (res.data?.files) {
    ...
    const handleBatchGenerate = async (rows: BatchNameEntry[]) => {
        ...
        try {
            ...
            const res = await axios.post(`${API_BASE}/api/generate_batch/tampa_bic`, form);
    ...
    return (
        <Layout title="Tampa BIC">
            ...
            <section className="flex-1 p-4 relative min-w-0 min-h-0 flex flex-col gap-3">
                ...
                {tmfUrl && (
                    <div className="flex-shrink-0 flex flex-col items-center gap-1">
                        ...
                        <button
                            onClick={() => downloadBlob(tmfUrl!, buildPrimaryFilename(params['text_line_1']))}
            ...
            <BatchGenerationModal
                ...
                onDownload={() => {
                    if (batchTmfUrl) downloadBlob(batchTmfUrl, 'tampa_bic_lote.zip');
                }}
