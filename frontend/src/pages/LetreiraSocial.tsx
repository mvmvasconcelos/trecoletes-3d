...
export default function LetreiraSocial() {
    const [config, setConfig] = useState<any>(null);
    const [params, setParams] = useState<Record<string, any>>({});
    const [isGenerating, setIsGenerating] = useState(false);

    const [baseUrl, setBaseUrl] = useState<string | null>(null);
    const [bordaUrl, setBordaUrl] = useState<string | null>(null);
    const [letrasUrl, setLetrasUrl] = useState<string | null>(null);
    ...
    const handleClearCache = () => clearCache(() => {
        setTmfUrl(null); setBaseUrl(null); setBordaUrl(null); setLetrasUrl(null);
        setBatchTmfUrl(null); setBatchProgress(null);
    });

    useEffect(() => {
        axios.get(`${API_BASE}/api/models/letreiro_social/config`)
            .then(res => {
                const cfg = res.data;
                setConfig(cfg);
                ...
                initial['extrusor_borda'] = 2;
                initial['extrusor_letras'] = 3;
    ...
    const handleGenerate = async () => {
        ...
        setError(null); setWarnings([]); setBaseUrl(null); setBordaUrl(null); setLetrasUrl(null); setTmfUrl(null); setFromCache(null);
        try {
            const form = new FormData();
            Object.entries(params).forEach(([k, v]) => form.append(k, String(v ?? '')));
            const res = await axios.post(`${API_BASE}/api/generate_parametric/letreiro_social`, form);
            if (res.data?.files) {
                if (res.data.files.base)   setBaseUrl(`${API_BASE}${res.data.files.base}`);
                if (res.data.files.borda)  setBordaUrl(`${API_BASE}${res.data.files.borda}`);
                if (res.data.files.letras) setLetrasUrl(`${API_BASE}${res.data.files.letras}`);
    ...
    const handleBatchGenerate = async (rows: BatchNameEntry[]) => {
        ...
        try {
            ...
            const res = await axios.post(`${API_BASE}/api/generate_batch/letreiro_social`, form);
    ...
    const renderParam = (p: any) => {
        ...
        switch (p.type) {
            ...
            case 'color': {
                const extField = p.id === 'base_color'   ? 'extrusor_base'
                               : p.id === 'border_color' ? 'extrusor_borda'
                               : 'extrusor_letras';
                const extDefault = p.id === 'base_color' ? 1 : p.id === 'border_color' ? 2 : 3;
                const extVal = params[extField] ?? extDefault;
                return (
                    <BambuColorPicker
    ...
    const bordaColor = (params['border_color'] as string) ?? '#2D7A3A';

    return (
        <Layout title="Letreiro Social">
                ...
                <div className="flex-1 relative min-h-0">
                    <div className="absolute inset-0">
                        <Viewer3D
                            ...
                            carimbArteUrl={letrasUrl}
                            cortadorUrl={null}
                            isGenerating={isGenerating}
                            artColor={(params['letters_color'] as string) ?? '#FFFFFF'}
                            modelColor={(params['base_color'] as string) ?? '#C8A46E'}
                            modelType="default"
                            highlightArte={thinWallParts.length > 0}
                            extraMeshes={bordaUrl ? [{ url: bordaUrl, color: bordaColor }] : []}
                        />
                    </div>
                </div>
                {tmfUrl && (
                    <div className="flex-shrink-0 flex flex-col items-center gap-1">
                        ...
                        <button
                            onClick={() => downloadBlob(tmfUrl!, 'letreiro_social_all.3mf')}
            ...
            <BatchGenerationModal
                ...
                onDownload={() => {
                    if (batchTmfUrl) downloadBlob(batchTmfUrl, 'letreiro_social_lote.zip');
                }}
                defaultExtrusorBase={Number(params['extrusor_base']) || 1}
                defaultExtrusorLetras={Number(params['extrusor_letras']) || 3}
