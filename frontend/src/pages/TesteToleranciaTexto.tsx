...
export default function TesteToleranciaTexto() {
    ...
    useEffect(() => {
        axios.get(`${API_BASE}/api/models/teste_tolerancia_texto/config`)
            .then(res => {
    ...
    // Dimensões estimadas (espelham o model.scad — layout vertical/Y)
    const gap = 2; // constante igual ao model.scad
    ...
    // Offset Y para texto_positivo no viewer: borda inferior da chapa (-plateD/2) - 10mm de gap - metade da altura do texto
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            form.append('n_chars', String(texto.trim().length || 1));

            const res = await axios.post(
                `${API_BASE}/api/generate_parametric/teste_tolerancia_texto`,
                form
            );
    ...
    return (
        <Layout title="Teste de Tolerância Texto">
            {/* ── Sidebar ─────────────────────────────────────────────────── */}
                ...
                <div className="p-4 space-y-6 flex-1">
                    ...
                    <div className="space-y-2">
                        ...
                        <input
                            ...
                            className="w-full bg-neutral-800 border border-neutral-700 rounded px-3 py-2 text-sm text-white focus:border-blue-500 focus:outline-none"
                        />
                    </div>
                    ...
                    <div className="space-y-2">
                        <label className="flex justify-between text-sm">
                            ...
                            <span className="text-blue-400 font-mono">{tamanho} mm</span>
                        </label>
                        <input
                            type="range"
                            min={5}
                    ...
                    <div className="space-y-2">
                        <label className="flex justify-between text-sm">
                            ...
                            <span className="text-blue-400 font-mono">{margem.toFixed(1)} mm</span>
                        </label>
                        <input
                            type="range"
                            min={1}
                    ...
                    <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-3 space-y-1.5">
                        ...
                        {[
                            ...
                            <div key={label} className="flex justify-between text-sm">
                                ...
                                <span className="text-blue-300 font-mono">{value.toFixed(1)} mm</span>
                            </div>
                        ))}
                        <div className="flex justify-between text-sm border-t border-neutral-800 pt-1.5 mt-1.5">
                            ...
                            <span className="text-blue-300 font-mono">{tolerancias.length}×</span>
                        </div>
                    </div>
            ...
            <section className="flex-1 p-4 relative min-w-0 min-h-0 flex flex-col gap-3">
                ...
                {(chapaUrl || textoUrl || tmfUrl) && (
                    <div className="flex-shrink-0 flex justify-center gap-3 flex-wrap">
                        ...
                        {tmfUrl && (
                            <button
                                onClick={() => downloadBlob(tmfUrl, 'teste_tolerancia_texto.3mf')}
